...
test("cli help", async () => {
  const output = await tsx("math", ["--help"]);
  expect(
    output.replace(
      /(commands:|flags:)/gi,
...
test("cli help add", async () => {
  const output = await tsx("math", ["add", "--help"]);
...
test("cli help divide", async () => {
  const output = await tsx("math", ["divide", "--help"]);
  expect(output).toMatchInlineSnapshot(`
    "divide v1.0.0
...
test("cli add", async () => {
  const output = await tsx("math", ["add", "1", "2"]);
  expect(output).toMatchInlineSnapshot(`"3"`);
});

test("cli add failure", async () => {
  const output = await tsx("math", ["add", "1", "notanumber"]);
  expect(output).toMatchInlineSnapshot(`
    "Validation error
...
test("cli divide", async () => {
  const output = await tsx("math", ["divide", "8", "4"]);
  expect(output).toMatchInlineSnapshot(`"2"`);
});

test("cli divide failure", async () => {
  const output = await tsx("math", ["divide", "8", "0"]);
  expect(output).toMatchInlineSnapshot(`
    "Validation error
...
test("migrations union type", async () => {
  ...
  expect(output).toContain("four: pending"); // Ensure correct behavior with union types
  expect(output).toMatchInlineSnapshot(`
    "one: executed
    two: executed
    three: executed
...
test("fs copy", async () => {
  ...
  `);
  expect(await tsx("fs", ["copy", "one", "uno", "--force"]))
    .toMatchInlineSnapshot(`
    "{
      "source": "one",
      "destination": "uno",
