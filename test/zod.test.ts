...
test("accepts strings", async () => {
  ...
  expect(acceptsString(z.intersection(z.string(), z.string().max(10)))).toBe(
    true
  );
});
...
test("accepts numbers", async () => {
  ...
  expect(acceptsNumber(z.intersection(z.number(), z.number().max(10)))).toBe(
    true
  );
});
...
test("accepts record", async () => {
  ...
  expect(acceptsRecord(z.intersection(z.record(z.string()), z.string()))).toBe(
    false
  );
  expect(
    acceptsRecord(z.intersection(z.record(z.string()), z.record(z.string())))
  ).toBe(true);
});
